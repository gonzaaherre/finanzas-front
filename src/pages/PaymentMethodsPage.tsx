    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-4xl">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">{paymentMethods.length} métodos de pago</p>
        </div>
        <button
          onClick={openNew}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
              {defaults.map(pm => (
                <div key={pm.id}
                  ...
                  <p className="text-sm font-medium text-gray-800 truncate">{pm.name}</p>
                  <div className="flex items-center gap-1 mt-0.5">
                    <Lock size={10} className="text-gray-400" />
                    <p className="text-xs text-gray-400">Predeterminado</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                {custom.map(pm => (
                  <div key={pm.id}
                    ...
                    <p className="text-sm font-medium text-gray-800 truncate min-w-0">{pm.name}</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input type="text" value={form.name}
                ...
                className={inputCls} placeholder="Ej: Billetera virtual" required />
            </div>
